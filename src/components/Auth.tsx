    ...
    <div className="h-screen bg-linear-to-br from-bg-main to-sidebar-bg flex items-center justify-center p-4 sm:p-8 relative overflow-hidden transition-colors">
      ...
      <div className="w-full max-w-5xl grid lg:grid-cols-2 gap-8 items-center z-10">
        <div className="hidden lg:flex flex-col space-y-6 pr-8">
          ...
          <div className="space-y-2">
            {[
              ...
              <div
                key={i}
                className="flex items-start gap-3 p-3 rounded-xl border border-transparent hover:border-border-ui/50 hover:bg-card-bg/80 dark:hover:bg-card-bg/50 hover:shadow-lg hover:shadow-accent/5 hover:scale-[1.02] transition-all duration-300 group cursor-default"
              >
        ...
        <div className="w-full flex flex-col items-center">
          <div className="w-full max-w-[400px] bg-card-bg rounded-2xl border border-border-ui shadow-2xl overflow-hidden transition-colors max-h-[90vh] flex flex-col">
            <div className="lg:hidden p-8 pb-0 flex flex-col items-center text-center">
              <div className="w-14 h-14 bg-white dark:bg-slate-50 rounded-xl flex items-center justify-center mb-4 shadow-lg border border-slate-200 dark:border-accent/10 p-2">
                <img src="/Logo-Vinance.png" alt="Vinance Logo" className="w-full h-full object-contain" />
              </div>
            ...
            <div className="p-6 sm:p-8 overflow-y-auto custom-scrollbar">
              <div className="flex p-1 bg-bg-main/80 dark:bg-bg-main/20 rounded-xl mb-6 border border-border-ui/50">
                <button
                  onClick={() => setIsLogin(true)}
                  className={cn(
                    "flex-1 py-3 text-sm font-bold rounded-xl transition-all duration-300",
                    isLogin ? "bg-card-bg text-accent shadow-sm" : "text-text-secondary"
                  )}
                >
                  Masuk
                </button>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  {success && (
                    <div className="p-3.5 rounded-xl bg-emerald-500/10 border border-emerald-500/20 flex items-center gap-3">
                      ...
                      <p className="text-xs text-emerald-500 font-medium leading-relaxed">{success}</p>
                    </div>
                  )}
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="relative group">
                        ...
                        <input
                          ...
                          className="w-full pl-11 pr-4 py-3.5 rounded-xl border border-border-ui bg-bg-main/50 text-text-primary outline-none focus:border-accent text-sm transition-all"
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="relative group">
                          ...
                          <input
                            ...
                            className="w-full pl-11 pr-4 py-3.5 rounded-xl border border-border-ui bg-bg-main/50 text-text-primary outline-none focus:border-accent text-sm tracking-[0.5em] font-bold transition-all"
                            placeholder="XXXXXX"
                          />
                        </div>
                      </div>
                      <button type="button" onClick={() => setRegStep(1)} className="text-[10px] font-bold text-accent hover:underline w-full text-center">Ganti email atau kirim ulang</button>
                    </div>
                  ) : (
                    <>
                      <div className="space-y-2">
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between items-center ml-1">
                          ...
                          {isLogin && (
                            <button type="button" onClick={() => setShowForgotModal(true)} className="text-[10px] font-bold text-accent hover:underline">Lupa Sandi?</button>
                          )}
                        </div>
                        <div className="relative group">
                          ...
                          <input
                            ...
                            className="w-full pl-11 pr-12 py-3.5 rounded-xl border border-border-ui bg-bg-main/50 text-text-primary outline-none focus:border-accent text-sm transition-all"
                            placeholder="••••••••"
                          />
                          <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 p-1 text-text-secondary hover:text-accent">
                  ...
                  <button
                    type="submit"
                    disabled={loading}
                    className="w-full py-4 bg-accent hover:bg-accent/90 text-white font-bold rounded-xl shadow-lg active:scale-[0.98] transition-all disabled:opacity-50"
                  >
          ...
          <div className="mt-6 flex flex-col items-center space-y-3">
            <p className="text-[10px] text-text-secondary uppercase tracking-[0.2em] font-bold text-center">Securely Synced with Google Sheets</p>
            <div className="flex gap-4">
              <div className="w-8 h-8 rounded-lg bg-card-bg border border-border-ui flex items-center justify-center shadow-sm">
                <img src="https://www.gstatic.com/images/branding/product/1x/sheets_2020q4_48dp.png" alt="Sheets" className="w-4 h-4" />
              </div>
              <div className="w-8 h-8 rounded-lg bg-card-bg border border-border-ui flex items-center justify-center shadow-sm">
                <img src="https://www.gstatic.com/images/branding/product/1x/apps_script_48dp.png" alt="Apps Script" className="w-4 h-4" />
              </div>
            </div>
          </div>
        </div>
      ...
      {showForgotModal && (
        <div className="fixed inset-0 z-[100] flex sm:p-4 items-center justify-center">
          <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm hidden sm:block" onClick={() => !forgotLoading && setShowForgotModal(false)} />
          <div className="bg-card-bg relative w-full h-full sm:h-auto sm:max-w-sm sm:rounded-2xl shadow-2xl p-6 border border-border-ui overflow-hidden z-10 text-center flex flex-col justify-center">
            <div className="w-14 h-14 bg-linear-to-br from-accent to-secondary rounded-full mx-auto mb-4 flex items-center justify-center shadow-lg">
              <Lock className="w-8 h-8 text-white" />
            </div>
            <h3 className="text-xl font-bold text-text-primary mb-2">Lupa Kata Sandi?</h3>
            <p className="text-xs text-text-secondary mb-5">{forgotStep === 1 ? "Masukkan email Anda untuk menerima kode verifikasi." : `Masukkan kode 6 digit yang dikirim ke email ${forgotEmail}.`}</p>
            {forgotStep === 1 ? (
              <form onSubmit={handleSendResetCode} className="space-y-4">
                <div className="relative group text-left">
                  <Mail className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-text-secondary" />
                  <input type="email" required value={forgotEmail} onChange={(e) => setForgotEmail(e.target.value)} className="w-full pl-11 pr-4 py-3 rounded-xl border border-border-ui bg-bg-main/50 text-text-primary outline-none focus:border-accent text-sm transition-all" placeholder="Masukkan email Anda" />
                </div>
                {forgotError && <p className="text-[10px] text-rose-500 font-bold text-left px-1">{forgotError}</p>}
                <button type="submit" disabled={forgotLoading} className="w-full py-3.5 bg-linear-to-r from-accent to-secondary text-white font-bold rounded-xl shadow-lg disabled:opacity-50 flex items-center justify-center gap-2">
              ...
              <form onSubmit={handleResetPassword} className="space-y-4">
                <div className="space-y-3">
                  <div className="relative group text-left">
                    <ShieldCheck className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-text-secondary" />
                    <input type="text" required maxLength={6} value={forgotCode} onChange={(e) => setForgotCode(e.target.value.replace(/\D/g, ''))} className="w-full pl-11 pr-4 py-3 rounded-xl border border-border-ui bg-bg-main/50 text-text-primary outline-none focus:border-accent text-sm tracking-[0.5em] font-bold" placeholder="XXXXXX" />
                  </div>
                  <div className="relative group text-left">
                    <Lock className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-text-secondary" />
                    <input type={showPassword ? "text" : "password"} required value={newResetPassword} onChange={(e) => setNewResetPassword(e.target.value)} className="w-full pl-11 pr-12 py-3 rounded-xl border border-border-ui bg-bg-main/50 text-text-primary outline-none focus:border-accent text-sm" placeholder="Kata sandi baru" />
                  </div>
                </div>
                {forgotError && <p className="text-[10px] text-rose-500 font-bold text-left px-1">{forgotError}</p>}
                <button type="submit" disabled={forgotLoading} className="w-full py-3.5 bg-linear-to-r from-accent to-secondary text-white font-bold rounded-xl shadow-lg disabled:opacity-50">Simpan Sandi Baru</button>
                <button type="button" onClick={() => setForgotStep(1)} className="text-[10px] font-bold text-accent hover:underline">Ganti email atau kirim ulang</button>
              </form>
            )}
            <button onClick={() => !forgotLoading && setShowForgotModal(false)} className="w-full mt-4 py-3 bg-bg-main border border-border-ui text-text-secondary font-bold text-sm rounded-lg disabled:opacity-50">Tutup</button>
          </div>
        </div>
      )}

      {showTermsModal && (
        <div className="fixed inset-0 z-[120] flex sm:p-4 items-center justify-center">
          <div onClick={() => setShowTermsModal(false)} className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm hidden sm:block" />
          <div className="relative w-full h-full sm:h-auto sm:max-w-lg bg-card-bg sm:rounded-2xl border border-border-ui shadow-2xl overflow-hidden flex flex-col max-h-full sm:max-h-[80vh] z-10">
            <div className="p-6 border-b border-border-ui flex items-center justify-between bg-bg-main/50">
              ...
              <button type="button" onClick={() => setShowTermsModal(false)} className="p-2 text-text-secondary"><X className="w-5 h-5" /></button>
            </div>
            <div className="p-6 overflow-y-auto space-y-6 text-sm text-text-secondary">
              <section className="space-y-3">
                <h4 className="text-xs font-black text-text-primary uppercase tracking-widest">1. Kepemilikan Data</h4>
                <p>Vinance menggunakan Google Sheets pribadi Anda sebagai database utama.</p>
              </section>
              <section className="space-y-3">
                <h4 className="text-xs font-black text-text-primary uppercase tracking-widest">2. Keamanan & Privasi</h4>
            ...
            <div className="p-6 bg-bg-main/50 border-t border-border-ui">
              <button type="button" onClick={() => setShowTermsModal(false)} className="w-full py-3.5 rounded-xl bg-accent text-white text-xs font-bold">Saya Mengerti</button>
            </div>
          </div>
        </div>
      )}
